...
function fileValidation({ wrapper, input, files }) {
  ...
  if (!acceptCheck(acceptedFile, files)) {
    updateOrCreateInvalidMsg(input, (wrapper.dataset.accept || 'The specified file type not supported.'));
  } else if (maxFileSize(fileSize, files)) {
    updateOrCreateInvalidMsg(input, (wrapper.dataset.maxFileSize || 'File too large. Reduce size and try again.'));
  } else if (multiple && maxItems !== -1 && files.length > maxItems) {
    updateOrCreateInvalidMsg(input, (wrapper.dataset.maxItems || `Specify a number of items equal to or less than ${maxItems}.`));
  } else if (multiple && minItems !== 1 && files.length < minItems) {
    updateOrCreateInvalidMsg(input, (wrapper.dataset.minItems || `Specify a number of items equal to or greater than ${minItems}.`));
  } else {
